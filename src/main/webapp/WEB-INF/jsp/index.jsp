...
<html lang="zh-CN">
  ...
  <body>
    ...
    <div class="footer">
      <p>个人版权信息</p>
      <p>
        成员一 学号：23H034160125 姓名 李晨豪 邮箱：419289657@qq.com
        电话：13857712790
      </p>
      <p>
        成员二 学号：23H034160105 姓名：蒋鈺 邮箱：3517460776@qq.com
        电话：15257468755
      </p>
    </div>
    ...
    <script>
      ...
      $(document).ready(function () {
        ...
        if (user.role === "admin") {
          $(".user-panel").prepend(
            '<a href="/admin" style="margin-right:15px; color:#1890ff; text-decoration:none;">进入后台</a>'
          );
        }
      ...
      function loadData(page) {
        ...
        $("#dataList").html(
          '<p style="grid-column: 1 / -1; text-align:center; color:#999;">加载中...</p>'
        );
        ...
        $.ajax({
          ...
          success: function (res) {
            if (res.code === 1) {
              ...
              $("#dataList").html(
                "<p style='grid-column: 1 / -1; color:red; text-align:center;'>加载失败：" +
                  res.msg +
                  "</p>"
              );
            }
          ...
          error: function () {
            $("#dataList").html(
              "<p style='grid-column: 1 / -1; color:red; text-align:center;'>服务器开小差了</p>"
            );
          },
        });
      }
      ...
      function renderGrid(list) {
        if (!list || list.length === 0) {
          $("#dataList").html(
            "<p style='grid-column: 1 / -1; text-align:center; padding:50px; color:#999'>暂无相关数据</p>"
          );
          return;
        }
        var html = "";
